...
# Removes 59 observations who reported mismatched zip code and state and 12 observations who did not report zip. (71 total)
...
d %>%
  dplyr::select(ResponseId, DemoState, DemoZip) -> l
...
sum(is.na(l$zip))  # 12 NAs are from missing zip code
sum(is.na(l$state))
...
l %>%
  dplyr::select(ResponseId, zip, invalidResponse, state, zip) %>%
  full_join(d, by = "ResponseId") -> d

# Removing observations with mismatched or missing zip codes
d %>%
